...
threads = []
pid = "/tmp/ioxfeederd.pid"
foreground = False
log = logging.getLogger()
log.setLevel(logging.DEBUG)
# create file handler which logs even debug messages
fh = logging.FileHandler('/tmp/ioxfeederd.log')
...
def main():
    global log
    log.info("ioxfeederd version " + constants.VERSION + " started")

    threads = []

    if "mqttInfluxdb" in configuration:
        thread = mqttInfluxdb(4, "mqttInfluxdb", configuration["mqttInfluxdb"])
        thread.start()
...
for opt, arg in opts:
    if opt in ("-h", "--help"):
        print('ioxfeederd.py -f -h')
        sys.exit()
    elif opt in ("-f", "--foreground"):
        foreground = True

daemon = IoxDaemon(app="ioxfeederd", pid=pid, action=main, chdir="./", logger=log, keep_fds=[fh.stream.fileno()],
